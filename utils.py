...
def summarize_audio_transcript_chunks(chunks, prompt_header, include_mentions, chunk_len_mins):
    ...
    if len(chunks) == 0:
        ...
        if include_mentions:
            mentioned = people_and_entities_mentioned_in_chunk(
                0, chunks[0], prompt_header)
        ...
        summary_of_summaries = summarize_the_summaries(
            summaries, prompt_header)
...
def summarize_text_chunks(chunks, prompt_header):
    ...
    if len(chunks) == 0:
        ...
        # Now we have the summaries, we can summarize the summaries
        summary_of_summaries = summarize_the_text_summaries(
            summaries, prompt_header)
